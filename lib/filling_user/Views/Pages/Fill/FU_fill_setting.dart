...
class FU_Fill_Setting extends StatefulWidget {
  ...
  FU_Fill_Setting(
      {required this.sizee, required this.qr_info, required this.id});

  @override
  _FU_Fill_SettingState createState() => _FU_Fill_SettingState();
}

class _FU_Fill_SettingState extends State<FU_Fill_Setting>
...
//  var device_numberl;
  ...
  initState() {
    ...
    fill_max = "FULL";
    fill_value = 25000;
    offset = Offset(0, widget.sizee.height * -0.06);
    super.initState();
  }

  //QR에서 가져온 정보(탱크ID)가 유효한지 확인하는 함수
  Future check_qr() async {
    qr_return = await FU_Http_services().post_qr(widget.qr_info, widget.id);
  }
